...
export async function getTransactions(username){
  const result = await db.query("SELECT transaction_name AS name, transaction_type AS type, transaction_amount AS amount, TO_CHAR(transaction_date, 'DD-MM-YYYY') AS date, category_name AS category FROM (SELECT transactions.transaction_name, transactions.transaction_type, transactions.transaction_amount, transactions.transaction_date, transactions.username, categories.category_name FROM transactions JOIN categories ON transactions.category_id = categories.category_id ORDER BY transaction_date DESC) AS records WHERE records.username =$2 ;",[username]);
